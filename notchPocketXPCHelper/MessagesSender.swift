...
enum MessagesSender {
    ...
    static func send(_ text: String, toChatNamed name: String) -> Bool {
        ...
        if let error {
            ...
            NSLog("[notchPocket] Messages send failed: \(error)")
            return false
        }
        ...
        switch status {
        case "ok-chat-1v1":
            NSLog("[notchPocket] Messages: sent into 1:1 chat with \(name.debugDescription) (account/service/handle/thread preserved)")
        case "ok-chat-group":
            NSLog("[notchPocket] Messages: no 1:1 chat for \(name.debugDescription); sent into first matching group chat")
        case "ok-participant":
            NSLog("[notchPocket] Messages: no chat matched \(name.debugDescription); sent to the single matching participant")
        case "notfound":
            NSLog("[notchPocket] Messages: no chat or participant named \(name.debugDescription)")
        case "ambiguous":
            NSLog("[notchPocket] Messages: \(name.debugDescription) matches multiple participants across handles/services — refusing to guess")
        default:
            NSLog("[notchPocket] Messages: unexpected status \(status.debugDescription) for \(name.debugDescription)")
        }
