...
final class NotificationWatcher {
    ...
    private let replyQueue = DispatchQueue(label: "NotchPocketXPCHelper.reply", qos: .userInitiated)
    ...
    private func refreshHeldBanners() {
        ...
        guard notchOpen else {
            for token in held where live[token] != nil && !skipLogged.contains(token) {
                ...
                NSLog("[notchPocket] notch closed — skipping keep-alive toggle for held banner \(token), leaving it to dismiss naturally")
            }
            return
        }
    ...
    private func collapseHeldBanners() {
        ...
        NSLog("[notchPocket] notch closed — collapsing \(expanded.count) held banner(s)")
    ...
    private func reply(token: String, text: String, on banner: AXUIElement) -> Bool {
        ...
        if let existing = replyField(in: banner) {
            ...
            if let action = rawAction(on: banner, matching: { $0.localizedCaseInsensitiveContains("reply") })
                ...
                NSLog("[notchPocket] reply failed for \(token): banner exposes no reply affordance")
                return false
            }
            guard let appeared = waitForReplyField(in: banner) else {
                NSLog("[notchPocket] reply failed for \(token): reply field never appeared after expanding the banner")
                return false
            }
            field = appeared
        }
        ...
        guard AXUIElementSetAttributeValue(field, kAXValueAttribute as CFString, text as CFString) == .success
            ...
            NSLog("[notchPocket] reply failed for \(token): could not set the reply field's value")
            return false
        }
        ...
        guard sendAccepted else {
            NSLog("[notchPocket] reply failed for \(token): the send action was not accepted")
            return false
        }

        guard verifyReplyAccepted(field: field) else {
            NSLog("[notchPocket] reply failed for \(token): banner did not accept the reply (field still filled after send)")
            return false
        }
        NSLog("[notchPocket] reply sent for \(token): banner accepted it (field cleared)")
        return true
    }
