...
final class AudioCaptureManager: ObservableObject {
    ...
    private let fftQueue = DispatchQueue(label: "com.jdylanmc.notchpocket.audiocapture.fft", qos: .userInitiated)
    private let ioQueue = DispatchQueue(label: "com.jdylanmc.notchpocket.audiocapture.io", qos: .userInteractive)
    private let lifecycleQueue = DispatchQueue(label: "com.jdylanmc.notchpocket.audiocapture.lifecycle", qos: .userInitiated)
    ...
    private func startCaptureOnLifecycleQueue(pids: [pid_t]) {
        ...
        let aggregateUID = "com.jdylanmc.notchpocket.audiotap.\(UUID().uuidString)"
        let aggregateDescription: [String: Any] = [
            kAudioAggregateDeviceNameKey: "notchPocketTapAggregate",
    ...
    private func performPermissionProbeOnLifecycleQueue() -> Bool {
        ...
        tapDescription.name = "Notch Pocket Audio Permission Probe"
        tapDescription.muteBehavior = .unmuted
        tapDescription.isPrivate = true
        ...
        let aggregateUID = "com.jdylanmc.notchpocket.permissionprobe.\(UUID().uuidString)"
        let aggregateDescription: [String: Any] = [
            kAudioAggregateDeviceNameKey: "Notch Pocket Audio Permission Probe",
            kAudioAggregateDeviceUIDKey: aggregateUID,
            kAudioAggregateDeviceMainSubDeviceKey: "",
            kAudioAggregateDeviceIsPrivateKey: 1,
            kAudioAggregateDeviceIsStackedKey: 0,
