...
final class SystemNotificationManager: ObservableObject {
    ...
    private func add(_ payload: [String: String]) {
        ...
        if bundleID == nil, let appName,
           let resolved = BundleIDResolver.shared.bundleID(forAppNamed: appName) {
            bundleID = resolved
            Log.notifications.debug("[notchPocket] resolved bundleID app-side: \(appName) -> \(resolved)")
        }
        ...
        guard isAllowed(notification) else {
            Log.notifications.debug("[notchPocket] filtered out: \(notification.appName ?? "-") bundle=\(notification.bundleID ?? "nil")")
            return
        }
    ...
    private func performReply(to notification: SystemNotification, text: String) async -> ReplyOutcome {
        ...
        if bannerDelivered == true {
            NSLog("[notchPocket] reply sent via AX for \(notification.appName ?? "-")")
            playSentSound()
            dismissActive(token: notification.id)
            return .sent
        }
        if bannerDelivered == nil {
            NSLog("[notchPocket] reply attempt timed out at the banner for \(notification.appName ?? "-")")
            return .failed
        }
        ...
        if notification.bundleID == "com.apple.MobileSMS",
           ...
           await raceTimeout(seconds: imessageScriptTimeout, fallback: false, operation: {
            ...
            NSLog("[notchPocket] reply sent via Messages scripting for \(chatName)")
            playSentSound()
            dismissActive(token: notification.id)
            return .sent
        }
        ...
        if notification.bundleID == "net.whatsapp.WhatsApp",
           ...
           let url = URL(string: "whatsapp://send?phone=\(phone)&text=\(encoded)") {
            NSLog("[notchPocket] reply drafted in WhatsApp conversation for \(sender)")
        ...
        NSLog("[notchPocket] reply could not be delivered (banner gone) — handing off to \(notification.appName ?? "app")")
