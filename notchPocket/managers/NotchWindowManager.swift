...
final class NotchWindowManager {
    ...
    struct ScreenContext {
        let viewModel: NotchPocketViewModel
        var window: NSWindow?
    ...
    let primaryViewModel = NotchPocketViewModel()
    ...
    var viewModels: [String: NotchPocketViewModel] {
    ...
    private func enableSkyLightOnAllWindows() {
        if Defaults[.showOnAllDisplays] {
            contexts.values.forEach { context in
                (context.window as? NotchPocketSkyLightWindow)?.enableSkyLight()
            }
        } else {
            (primaryWindow as? NotchPocketSkyLightWindow)?.enableSkyLight()
        }
    }
    ...
    private func disableSkyLightOnAllWindows() {
        ...
        Task {
            ...
            await MainActor.run {
                if Defaults[.showOnAllDisplays] {
                    contexts.values.forEach { context in
                        (context.window as? NotchPocketSkyLightWindow)?.disableSkyLight()
                    }
                } else {
                    (primaryWindow as? NotchPocketSkyLightWindow)?.disableSkyLight()
                }
            }
        }
    }
    ...
    func cleanupWindows(shouldInvert: Bool = false) {
        ...
        NotchPocketViewCoordinator.shared.applyOSDSources()
    }

    private func createNotchPocketWindow(for screen: NSScreen, with viewModel: NotchPocketViewModel) -> NSWindow {
    ...
    func adjustWindowPosition(changeAlpha: Bool = false) {
        let coordinator = NotchPocketViewCoordinator.shared
        if Defaults[.showOnAllDisplays] {
            ...
            for screen in NSScreen.screens {
                ...
                if contexts[uuid] == nil {
                    contexts[uuid] = ScreenContext(
                        viewModel: NotchPocketViewModel(screenUUID: uuid),
                        window: nil,
                        dragDetector: nil
                    )
                }
                ...
                if contexts[uuid]?.window == nil {
                    ...
                    let window = createNotchPocketWindow(for: screen, with: viewModel)
            ...
            if primaryWindow == nil {
                primaryWindow = createNotchPocketWindow(for: selectedScreen, with: primaryViewModel)
            }
    ...
    func setupDragDetectors() {
        ...
        if Defaults[.showOnAllDisplays] {
            ...
            let preferredScreen: NSScreen? = primaryWindow?.screen
                ?? NSScreen.screen(withUUID: NotchPocketViewCoordinator.shared.selectedScreenUUID)
                ?? NSScreen.main
    ...
    private func handleDragEntersNotchRegion(onScreen screen: NSScreen) {
        guard Defaults[.notchPocketShelf] else { return }
        guard let uuid = screen.displayUUID else { return }
    ...
    func prepareInitialWindows() {
        if !Defaults[.showOnAllDisplays] {
            ...
            if let screen = NSScreen.main ?? NSScreen.screens.first {
                primaryWindow = createNotchPocketWindow(for: screen, with: viewModel)
            }
            adjustWindowPosition(changeAlpha: true)
        } else {
            adjustWindowPosition(changeAlpha: true)
