    ...
    private func showOSD(for keyType: NXKeyType, command: Bool) {
        Task { @MainActor in
            ...
            case .soundUp, .soundDown, .mute:
                ...
                NotchPocketViewCoordinator.shared.toggleSneakPeek(status: true, type: .volume, value: CGFloat(v))
            case .brightnessUp, .brightnessDown:
                if command {
                    let v = KeyboardBacklightManager.shared.rawBrightness
                    NotchPocketViewCoordinator.shared.toggleSneakPeek(status: true, type: .backlight, value: CGFloat(v))
                } else {
                    let v = BrightnessManager.shared.rawBrightness
                    let target = await BrightnessManager.shared.brightnessTargetUUID()
                    NotchPocketViewCoordinator.shared.toggleSneakPeek(status: true, type: .brightness, value: CGFloat(v), targetScreenUUID: target)
                }
            case .keyboardBrightnessUp, .keyboardBrightnessDown:
                let v = KeyboardBacklightManager.shared.rawBrightness
                NotchPocketViewCoordinator.shared.toggleSneakPeek(status: true, type: .backlight, value: CGFloat(v))
