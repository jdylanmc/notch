...
final class XPCHelperClient: NSObject, ObservableObject {
    ...
    private var remoteService: RemoteXPCService<NotchPocketXPCHelperProtocol>?
    ...
    private func ensureRemoteService() -> RemoteXPCService<NotchPocketXPCHelperProtocol> {
        ...
        let service = RemoteXPCService<NotchPocketXPCHelperProtocol>(
            connection: conn,
            remoteInterface: NotchPocketXPCHelperProtocol.self
        )
    ...
    private func makeAppDelegateInterface() -> NSXPCInterface {
        let interface = NSXPCInterface(with: (any NotchPocketXPCAppDelegate).self)
        interface.setClasses(
            NSSet(array: [BNLunarBrightnessEvent.self]) as! Set<AnyHashable>,
            for: #selector(NotchPocketXPCHelperLunarListener.lunarEventDidUpdate(_:)),
            argumentIndex: 0,
            ofReply: false
        )
        return interface
    ...
    func startLunarEventStream(listener: NotchPocketXPCHelperLunarListener) async -> Bool {
...
final class NotificationXPCDelegate: NSObject, NotchPocketXPCAppDelegate {
    ...
    private var _lunarListener: NotchPocketXPCHelperLunarListener?

    var lunarListener: NotchPocketXPCHelperLunarListener? {
        get {
    ...
    func notificationDidAppear(_ payload: [String: String]) {
        NSLog("[notchPocket] app received banner: \(payload["appName"] ?? "-") / \(payload["title"] ?? "-")")
        NotificationCenter.default.post(
    ...
    nonisolated func notchClosed() {
        Task { @MainActor in
            guard notchOpenCount > 0 else {
                NSLog("[notchPocket] XPCHelperClient: unmatched notchClosed ignored")
                return
            }
            notchOpenCount -= 1
