...
metadata:
    github-path: skills/swiftui-expert-skill
    github-pinned: 4c6a97d15aa5e023538c3cb06b5192f241dd451d
    github-ref: 4c6a97d15aa5e023538c3cb06b5192f241dd451d
    github-repo: https://github.com/AvdLee/SwiftUI-Agent-Skill
    github-tree-sha: 7bca871fd00732b40626acbce1f3a509a4f5b849
...
Read [AGENTS.md](../../../AGENTS.md) and the current implementation first.
Use this skill as expertise for the assigned task, not permission to modernize
unrelated code or add features.

- The deployment target is **macOS 14**, distinct from the macOS 15.6+/Xcode 26+
  build host. Check macOS availability and fallbacks explicitly; an
  `#available(iOS 26, *)` example does not guard a macOS API.
- Preserve the existing AppKit Settings controller, panel/display lifecycle,
  centralized `Defaults`, observable models, and every Settings pane. Do not
  replace them merely because a reference prefers a newer SwiftUI pattern.
- Preserve app/helper bundle identities, media and shelf data, and the
  Spotify-only support commitment. Use the repository's build/test/lint scripts
  only when execution is authorized. Never use signing bypasses as repair.
- No updater/feed, privacy changes, automatic publication/merge, or unsolicited
  Liquid Glass adoption. Use the [notch skill](../notch/SKILL.md) for separately
  authorized app-window capture; never substitute a display capture.
- Packaged trace scripts are optional upstream tools, not an installation or
  verification step. Do not run them without separate approval naming the
  exact app target and private local output. Do not record all processes.
  Keep traces/logs local; neither availability nor a script flag grants consent.

See the [vetting and update record](../README.md) and [MIT notice](LICENSE).
The upstream references and scripts below are retained, not executed on import.
