...
metadata:
    github-path: macos-patterns
    github-pinned: a60365ae85bfc3d1f2f8b260b080d77bfb2f3ec0
    github-ref: a60365ae85bfc3d1f2f8b260b080d77bfb2f3ec0
    github-repo: https://github.com/fayazara/macos-app-skills
    github-tree-sha: 8d729c6b4616b233b7a3ed322d9a8bfc7d7ef269
name: macos-patterns
license: MIT
---
# Native macOS Patterns for Web Developers

## notchPocket compatibility guard

This repository adaptation takes precedence over conflicting examples below.
Read [AGENTS.md](../../../AGENTS.md) and the relevant current code first.
The examples are conceptual references, not drop-in implementations or
authorization to change application behavior.

- Preserve macOS 14 deployment and use macOS-specific availability guards.
  Do not confuse the newer build host with the minimum supported OS.
- Keep `SettingsWindowController.shared.showWindow()`, the
  `NotchPocketSettingsWindow` identifier, existing Settings panes,
  `NotchPocketSkyLightWindow`, and per-display lifecycle responsibilities.
  Preserve centralized `Defaults` and the existing `LaunchAtLogin` integration;
  do not replace them with raw `@AppStorage` or `SMAppService` examples.
- The ScreenCaptureKit display example below is **not permitted for notchPocket
  debugging**. Use the [notch skill](../notch/SKILL.md) and its owned-window
  selector, permission preflight, exclusion checks and private local output.
  `sharingType = .none` alone is not proof of exclusion on every OS.
- Do not install monitors/hotkeys, modify clipboard contents, change login or
  privacy settings, or run capture examples merely to demonstrate this skill.
  Those are separate, scoped actions, not expertise installation.
- The helper is a bundled application XPC service, not a privileged Mach
  service. Preserve app/helper identities and existing media/shelf data.
  `SparkleView` is decorative; no updater/feed is authorized. Signing,
  notarization, publication and merges remain separately human-approved.

Use the repository's build/test/lint scripts only when the task authorizes
execution. See the [vetting and update record](../README.md) and the preserved
[upstream license declaration](LICENSE).
