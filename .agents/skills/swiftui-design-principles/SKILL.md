...
- Preserve macOS 14 deployment, AppKit window/panel ownership and centralized
  `Defaults`. UIKit colors, iOS navigation-bar APIs and lock-screen widget
  families below are not drop-in macOS implementations.
- Notch Pocket's Dashboard widgets are in-app views, not WidgetKit extensions.
  This skill does not authorize new targets, entitlements, timelines or shared
  storage. WidgetKit-specific budgets and layouts are not app-wide limits.
- Treat spacing, typography, corner sizes and navigation prescriptions as
  context-dependent design guidance. Preserve approved compact geometry,
  necessary custom headers and existing behavior unless redesign is requested.
- Use semantic colors and native controls appropriate to macOS; reuse existing
  helpers and validate adapted snippets. Installation changes no app code,
  permissions, persistent data, signing, updater or release behavior.
