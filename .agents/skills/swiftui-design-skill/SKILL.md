...
## Notch Pocket compatibility guard

This repository guard takes precedence over conflicting upstream advice below.
Read [AGENTS.md](../../../AGENTS.md) and the
[integration record](../README.md#swiftui-design-additions) before applying it.

- Preserve macOS 14 deployment and the existing AppKit/SwiftUI/Defaults
  architecture. SDK prerequisites and iOS availability checks do not establish
  macOS availability. Do not remove necessary `NSColor` bridges.
- Start from the approved compact panel geometry, interactions and existing
  design context. Typography, spacing, touch-target and decoration rules below
  are design proposals, not authority to redesign the app or remove
  `GeometryReader` where its geometry is needed.
- The bundled extensions are examples, not compile-validated app dependencies.
  Some use UIKit-only APIs. Reuse existing helpers; validate platform support,
  types and invalid-input behavior before adapting individual examples.
- Installing this skill does not authorize brand/font downloads, new repository
  design documents, application control, privacy changes or publication.
  Use the existing `notch` skill for separately authorized UI control.
