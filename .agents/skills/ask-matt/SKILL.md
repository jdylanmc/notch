...
- **`/prototype`** is a small Swift model or SwiftUI fixture that answers one design question. Keep it in an approved isolated worktree on a `feature/prototype-<name>` branch based on `origin/pocket`; preserve it as a primary source rather than merging prototype controls into the product. New-feature prototypes still respect #54 and the current task's execution authority. It's the detour in step 2 of the main flow, but reach for it any time a design question is hard to settle on paper.
